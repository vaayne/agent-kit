...
**Cause:** `mori pane send` only reliably delivers to the window's _active_ pane. The `--pane` flag is accepted but observed to drop in testing when the target is not active.
...
**Cause:** `mori pane message` routes to the window's _active_ agent — if you clicked into a different pane, the active agent shifted.
