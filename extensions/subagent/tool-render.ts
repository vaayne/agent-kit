...
function formatModelLabel(overrides?: { model?: string; thinking?: ThinkingLevel }): string {
  if (!overrides?.model) {
    return "";
  }
...
function renderSingleResult(details: SubagentDetails, expanded: boolean, theme: ThemeLike) {
  ...
  if (expanded) {
    ...
    if (result.sessionId) {
      container.addChild(new Text(theme.fg("dim", `Session: ${result.sessionId}`), 0, 0));
    }
...
function renderChainResult(details: SubagentDetails, expanded: boolean, theme: ThemeLike) {
  ...
  if (expanded) {
    ...
    for (const result of details.results) {
      ...
      container.addChild(
        new Text(theme.fg("muted", "Prompt: ") + theme.fg("dim", result.task), 0, 0),
      );
      for (const item of displayItems) {
        if (item.type !== "toolCall") continue;
        container.addChild(
...
function renderParallelResult(details: SubagentDetails, expanded: boolean, theme: ThemeLike) {
  ...
  for (const result of details.results) {
    ...
    if (displayItems.length === 0) {
      const emptyState = result.exitCode === -1 ? "(running...)" : "(no output)";
