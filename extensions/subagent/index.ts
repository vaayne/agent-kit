...
interface AgentCommandResultDetails {
	agent: string;
	agentSource: "user" | "project" | "unknown";
	task: string;
	output: string;
...
function isCommandSafeAgentName(name: string): boolean {
	return /^[A-Za-z0-9._-]+$/.test(name);
}

function splitAgentTask(args: string): { agent: string; task: string } | null {
...
export default function (pi: ExtensionAPI) {
	...
	const sendAgentCommandResult = (result: SingleResult, task: string) => {
		const output =
	...
	const registerAgentCommands = (cwd: string) => {
		...
		for (const agent of discovery.agents) {
			...
			pi.registerCommand(commandName, {
				description: `Run a task with the ${agent.name} subagent`,
				handler: async (args, ctx) => {
					...
					const currentDiscovery = discoverAgents(ctx.cwd, "both");
					const currentAgent = currentDiscovery.agents.find(
						(candidate) => candidate.name === agent.name,
					);
					if (!currentAgent) {
						ctx.ui.notify(`Unknown agent: ${agent.name}`, "error");
						return;
					}
					...
					const ok = await confirmProjectAgentIfNeeded(
						ctx,
						currentAgent,
						currentDiscovery.projectAgentsDir,
					);
					if (!ok) return;

					const result = await runSingleAgent(
						ctx.cwd,
						currentDiscovery.agents,
						currentAgent.name,
						task,
						undefined,
						undefined,
						...
						(results) => ({
							mode: "single",
							agentScope: "both",
							projectAgentsDir: currentDiscovery.projectAgentsDir,
							results,
						}),
					);

					sendAgentCommandResult(result, task);
				},
			});
		}
	};
	...
	pi.registerCommand("agents", {
		description: "Run a task with a named subagent: /agents <agent> <task>",
		getArgumentCompletions: (prefix) => {
			const items = discoveredAgents
				.map((agent) => ({
					value: `${agent.name} `,
		...
		handler: async (args, ctx) => {
			...
			sendAgentCommandResult(result, parsed.task);
		},
	});
