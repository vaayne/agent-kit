...
- Unfiled 桶：没绑 task 的线程，放"其它"段最末。可以一键"归入 task"或"变成 task"。
...
同一页第二个 tab：**全景**。这是"全局看一眼"的视图，给每天开始和每周 review
两个时刻用：

- 一屏所有未完成 task，按 `waitingOn` 分四组，固定顺序：**等你**、**在跑**、
  **停了没 next**、**等 CI / 等别人**。第三组单独拎出来，因为它就是今天
  漂掉的那类（31 个卡在 in_review），不单独显示就永远不会被处理。
- 每行 key、标题、`next`、多久没动。顶部 project chip 过滤。
- 没有列、没有拖拽、不横向滚动。按 status 分列回答"在流水线哪一段"，
  那是漂掉的字段；按 `waitingOn` 分组回答"谁该动"，那才是看一眼要的答案。
- done 不显示，只有一个折叠的"本周完成 N 件"。回指：完成感。
- 一个"归档 30 天没动的"批量动作。第一次打开时"停了没 next"会装满旧 task，
  没有这个动作会吓退人。
...
- 不做 status 分列的看板。bb Tasks 自带的那页保留，需要时开它；本 plugin 的
  全局视图是 4.3 的全景 tab，按 `waitingOn` 分组而不是按 status 分列。
