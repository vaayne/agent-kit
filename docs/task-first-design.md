...
- 分段固定顺序：**轮到你**、**在跑**、**临时**、**等 CI / 等别人**、**停了**、**未开始**、
  **最近完成**。前两段默认展开，其余默认折叠只显示数量。折叠段的名字和全景看板的列一致，
  不再用"其它"这种看不出用途的桶。回指：视野外即不存在的反面，不需要你的事主动消失。
...
- **临时**段：最近 7 天没绑 task 的根线程，按更新时间倒序，独立一段放在"在跑"之后、
  默认折叠只显数量。hover 一行可"提升为 task"。只看 7 天：它是提醒你归档，不是全部历史。
