...
侧栏只回答一个问题：我现在该点哪儿。三层，默认只有这些：

- **PMO 一行**：常驻 PMO 线程，虚线框，不归任何 task。设置 `pmoThreadId` 为空则不显示。
- **现在（Now）**：需要你的（agent 在问、线程出错、PR 等你 review 或 CI 失败）和在跑的 task，
  外加你当前所在线程所属的 task（不管它推导到哪一态，你在里面它就在这里）。每行：状态点、
  key、标题、一个理由词、多久没动。空了整段消失，不显示"没有"。
  状态图标是 12px 手绘 SVG，一套线宽，按紧急度只显示一个：琥珀问号（agent 在问）、红叹号
  （线程出错）、旋转弧（在跑，唯一会动的）、圆点（上次打开后有新动静）。闲置且已读什么都不画。
- **临时（Scratch）**：最近 7 天没绑 task 的根线程，按更新时间倒序。一次性小活就住这里，
  不需要 task；hover 出"提升为 task"。默认 6 条，可展开全部。
- **更多（More）**：折叠，不显示计数（数字不指向动作，只制造焦虑）。展开后是小标题分组：
  等 CI / 等别人、停了、未开始、最近完成。搜索时自动展开。

不再有 project chips：每行 key 前缀就是 project，搜索 `AK-` 即过滤。project 过滤留在全景看板。
展开一个 task 显示线程树（父子缩进）和 PR 一行；归档线程变灰但仍可点开。
超过 3 天没动的用暗色，不用红色。回指：时间盲，且不制造焦虑。
底部一行 usage（"Claude 42% · Codex 6%"），贴着设置按钮上方，点开看每个窗口。
没有计数徽章，没有 provider 名，没有未读点。

**语言**：SDK 没有 locale API。所有文案在 `src/strings.ts`（zh / en），设置 `language` 选择，
`auto` 跟浏览器。推导出的理由是代码（`asking`、`ciFailed`…），每个面按语言渲染。
