...
RPC = os.environ.get(
    "TASK_NAVIGATOR_RPC", "http://127.0.0.1:38886/api/v1/plugins/task-navigator/rpc"
)
NEXT_MAX_DAYS = 3
STALE_DAYS = 30
DAY_MS = 86_400_000
...
def primary(task):
    ...
    asking = [
        t
        for t in threads
        if t["status"] in ("pendingInteraction", "error") and not t["archived"]
    ]
    if asking:
        return asking[0]
...
def main():
    ...
    ap.add_argument(
        "--apply",
        action="store_true",
        help="perform rule 1 (mark merged-and-idle tasks done)",
    )
    ...
    for t in active:
        prs = t["pullRequests"]
        live = [
            x for x in t["threads"] if not x["archived"] and x["status"] == "running"
        ]
        if (
            prs
            and all(p["state"] == "merged" for p in prs)
            and not live
            and t["status"] != "done"
        ):
        ...
        if t["group"] == "stalled":
            ...
            (old_stalled if md is not None and md > STALE_DAYS else stalled).append(
                (t, md)
            )
    ...
    if args.apply:
        for t in merged_done:
            ...
            bb(
                ...
                "PMO",
                "--body",
                f"PMO：PR {numbers} 已合并且没有线程在跑，标记为 done。",
            )
            ...
            json.dumps(
                {
                    ...
                    "mergedDone": [t["key"] for t in merged_done],
                    "applied": applied,
                    "nextExpired": [
                        {"key": t["key"], "next": t["next"], "days": round(d, 1)}
                    ...
                    "stalled": [
                        {
                            "key": t["key"],
                            "title": t["title"],
                            "thread": (primary(t) or {}).get("id"),
                            "days": None if d is None else round(d, 1),
                        }
                        for t, d in stalled
                    ],
                    "oldStalled": [
                        {"key": t["key"], "title": t["title"], "days": round(d, 1)}
    ...
    print(
        f"等你 {c['you']} · 在跑 {c['running']} · 等 CI/别人 {c['waiting']} · 停了 {c['stalled']} · 未开始 {c['backlog']} · 本周完成 {ov['doneThisWeek']}"
    )
    if ov.get("pmo") is None:
        print("(提示：plugin 设置 pmoThreadId 为空，侧栏不会显示 PMO 行)")
    print()
    print(
        f"## PR 已合并 → done ({len(merged_done)})"
        + ("  [已执行]" if args.apply else "  [未执行，加 --apply]")
    )
    for t in merged_done:
    ...
    for t, d in stalled:
        p = primary(t)
        print(
            f"- {t['key']} {t['title']} · {'' if d is None else f'{d:.0f} 天没动'} · thread {p['id'] if p else '无'} · {t['status']}"
        )
